...
```{r}
wildfire_shp_data <- st_read("data/LANDSAT_USA_contiguous_and_Hawaii_7d/LANDSAT_USA_contiguous_and_Hawaii_7d.shp")
```
...
```{r}
fires_by_confidence <- wildfire_shp_data %>% 
  group_by(CONFIDENCE) %>% 
  summarise(number_of_fire = n())
```
...
```{r}
ggplot(data = fires_by_confidence, aes(x = CONFIDENCE, y = number_of_fire, fill = CONFIDENCE))+
...
```{r}
wildfire_csv_data <- read_csv("data/LANDSAT_USA_contiguous_and_Hawaii_7d.csv")
```

```{r}
wildfire_sf <- st_as_sf(wildfire_csv_data, coords = c("latitude", "longitude"),crs=4326)
```
...
```{r}
# Basic map
wildfire_sf <- st_transform(wildfire_sf, st_crs(usa))
p <- ggplot() +
  geom_sf(data = usa, fill = "gray90", color = "gray70") +  # basemap layer
  geom_sf(data = wildfire_sf, aes(color = confidence), alpha = 0.6) +
coord_sf(xlim = c(-125, -66), ylim = c(24, 50)) +
  theme_minimal()
  # labs(title = "Wildfires in the USA and Hawaii",
  #      subtitle = "Date: {frame_time}",
  #      color = "Size (log scale)",
  #      size = "Size") +
