...
When switching to step_equal_interval( ) and step_jenks( ) could be both balanced. For equal interval, it could be balanced visually because it divides the range into evenly spaced numeric breaks. For Jenks, it could be balanced contextually/statistically because groups similar values together naturally.
...
hi_median_income <- hi_median_income |>
  mutate(
    poverty_flag = ifelse(estimate < 107689, "Below Line", "Above Line")
  )
```
...
```{r}
# Example: Households with Internet in New Mexico
nm_internet <- get_acs(
  geography = "tract",
...
nm_scale <- interpolate_palette(
  data = nm_internet,
  column = "estimate",
  method = "equal",
  n = 4,
  palette = viridisLite::magma  # Feel free to change palette
)

mapboxgl(bounds = nm_internet) |>
  add_fill_layer(
    id = "nm_tracts",
    source = nm_internet,
    fill_color = nm_scale$expression,
    fill_opacity = 0.6
  ) |>
  add_legend(
    "Households with Internet in New Mexico",
    values = get_legend_labels(nm_scale, digits = 0),
    colors = get_legend_colors(nm_scale),
    type = "continuous"
  )
```
